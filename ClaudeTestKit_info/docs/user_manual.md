...
現在ロードされている ClaudeTestKit のバージョン文字列を返します。最新リリース (2026-04-30) では `"2026-04-30-phase-f-prevalidate-v2"` が設定されており、Phase F の PreValidate 拡張を反映した版であることを示します。

```wolfram
$ClaudeTestKitVersion
(* "2026-04-30-phase-f-prevalidate-v2" *)
```

> **注記 (2026-04-29 以降):** パッケージのロード時に表示されていた「ClaudeTestKit パッケージがロードされました。」のメッセージは廃止されました。バージョン情報を確認したい場合は、明示的に `$ClaudeTestKitVersion` を評価してください。
...
#### PreValidate 拡張 (Phase F — 2026-04-30 以降)

`CreateMockAdapter` / `CreateMockTransactionAdapter` が返す mock adapter には、実 `ClaudeBuildTransactionAdapter` / `ClaudeBuildRuntimeAdapter` と等価な **PreValidate フック** が組み込まれています。proposal が validation 本体に渡る前に、以下の 4 ケースを早期に弾きます。

| # | 条件                                              | Decision        | ReasonClass            | フラグ                           |
|---|--------------------------------------------------|-----------------|------------------------|----------------------------------|
| 1 | コード文字列が空、または 10 文字未満             | `RepairNeeded`  | `EmptyOrTrivialCode`   | `EmptyCodeDetected -> True`      |
| 2 | メタ関数ラッパー呼び出しを検出                   | `Deny`          | `MetaCallProposal`     | `MetaCallDetected -> True`       |
| 3 | `HeldExpr` が `HoldComplete[Null]`               | `RepairNeeded`  | `NullCodeProposal`     | `NullCodeDetected -> True`       |
| 4 | コメント・空白を除去すると 5 文字未満            | `RepairNeeded`  | `NullCodeProposal`     | `NullCodeDetected -> True`       |

検出されるメタ関数ラッパーは次の通りです（ケース 2）。これらを直接 worker proposal の `RawCode` に含めると `Deny` されます。

```
ClaudeUpdatePackage         ClaudeUpdatePackageViaRuntime
ClaudeBuildTransactionAdapter
ClaudeBuildRuntimeAdapter
ClaudeRunTurn               CreateClaudeRuntime
ClaudeStartRuntime
ClaudeEval                  ClaudeCreatePackage
ContinueUpdate              ContinueEval
```

PreValidate を経由した proposal はその場で結果を返すため、後続の `Validate` フック（`AllowedHeads` / `DenyHeads` の照合等）は呼ばれません。これを利用して、テストでは「不正コード proposal を投入したときに正しい `ReasonClass` が記録されるか」を `AssertOutcome` / `AssertValidationDenied` と組み合わせて検証できます。

返り値の Association には診断用の追加キーも含まれます：

- `"SanitizedExpr"`         — そのまま転送した `HeldExpr`
- `"CodeLength"`            — `StringTrim` 後のコード長
- `"CodeAfterCommentStrip"` — コメント・連続空白を 1 文字に正規化した残り
- 例: 空コード時は説明文 `"Empty or trivial code (length=N)"` が `Decision` に併記されます。

> **互換性:** 旧来の mock adapter で許可されていた「PreValidate ステージを通り抜ける trivial proposal」は、新しい挙動では拒否されます。意図的に空コードを投入してアサーションを書いていたテストは、期待する `Decision` を `"RepairNeeded"` / `"Deny"` に更新してください。
...
オプションは `CreateMockAdapter` と同様で、上記の PreValidate 拡張も同じく適用されます。
...
PreValidate 拡張により、メタ関数ラッパー呼び出しを含む proposal も `Deny` として記録されるため、このアサーションで一括検証できます。
...
Phase 32 で導入された、Committer の自然言語 Input 流入防止 (Task 3.1) と Auto ゲート強化 (Task 3.2) に対応するテスト関数群です。

---

### Task 3.1: Committer 自然言語 Input 流入防止

#### `RunPhase32Task31Tests`

`iValidateWorkerProposal` (role="Commit") と `iIsPlausibleInputCellContent` / `iHeldExprInputCellStrings` の単体テストを実行し、PASS/FAIL サマリを返します。Committer が `Cell["自然言語", "Input"]` を書き込もうとしないことを保証します。
...
`HoldComplete[NotebookWrite[nb, Cell["日本語文", "Input"]]]` を role="Commit" で投入した際に、`Decision == "Deny"` かつ `ReasonClass == "NaturalLanguageInInputCell"` が返り、`OffendingStrings` が空でないことを検証します。

#### `AssertPhase32Task31AcceptsMathExprInInputCell`

`HoldComplete[NotebookWrite[nb, Cell["Plot[Sin[x], {x, 0, 2 Pi}]", "Input"]]]` を role="Commit" で投入した際に、`Decision == "Permit"` かつ `ReasonClass == "OK"` が返ることを検証します。

#### `AssertPhase32Task31AcceptsNaturalLangInTextCell`

`HoldComplete[NotebookWrite[nb, Cell["日本語文", "Text"]]]` を role="Commit" で投入した際に `Decision == "Permit"` が返ることを検証します（"Text" スタイルは検査対象外）。
...
`iIsShortFactualQuery` と `iHasComplexTaskMarker` の単体テストを実行し、PASS/FAIL サマリを返します。Auto モードで短い factual query を Single に fallback させ、複雑タスクを Orchestrator に通すゲートを検証します。
...
短い日本語 factual query（例: 「`$packageDirectory`の claudecode…を調べて」）に対して、`iIsShortFactualQuery` が `True`、`iHasComplexTaskMarker` が `False` を返すことを検証します。

#### `AssertPhase32Task32SkipsShortFactualEn`

短い英語 factual query（例: "Check if claudecode.wl is newer than its GitHub version"）に対して、`iIsShortFactualQuery` が `True` を返すことを検証します。

#### `AssertPhase32Task32KeepsLongComplex`

長いマルチステップタスク（600 文字以上、複数の順序接続語を含む）に対して、`iHasComplexTaskMarker` が `True`、`iIsShortFactualQuery` が `False` を返し、Orchestrator 経路を維持することを検証します。

#### `AssertPhase32Task32KeepsSlideRequest`

スライド・レポート等の成果物要求（例: 「30 ページのスライドを作って」）に対して `iHasComplexTaskMarker` が `True` を返すことを検証します。

#### `AssertPhase32Task32KeepsSequentialTask`

順序接続語が 2 個以上（「まず…次に…最後に…」）あるタスクで `iHasComplexTaskMarker` が `True` を返すことを検証します。
...
## PreValidate を意識したワークフロー (Phase F)

PreValidate 拡張によって弾かれる proposal を意図的に投入し、`ReasonClass` が正しく付与されるかを確認するパターンです。

```wolfram
(* メタ関数ラッパー呼び出しを worker に提案させる (Deny を期待) *)
adapter = CreateMockAdapter[
  "Provider" -> CreateMockProvider[{
    "ClaudeUpdatePackage[\"foo\", \"bar\"]"   (* meta-call *)
  }],
  "AllowedHeads"     -> {NBCellRead},
  "ExecutionResults" -> {},
  "MaxContinuations" -> 1
...
result = RunClaudeScenario[<|
  "Name"       -> "MetaCallRejection",
  "Input"      -> "meta wrapper を呼ぶ proposal",
  "Adapter"    -> adapter,
  "Profile"    -> <|"MaxTokens" -> 200|>,
  "Assertions" -> {
    AssertOutcome[#, "Denied"] &,
    AssertValidationDenied[#] &
  }
|>];

result["Passed"]   (* True — PreValidate が MetaCallProposal を Deny *)
```

空コードや `HoldComplete[Null]` の proposal を投入した場合は、`Decision == "RepairNeeded"` が記録され、後続の repair ループ動作も検証できます。
...
| 関数 | 目的 |
|------|------|
| `AssertT05SlideExtraction[]` | `iExtractSlidesFromPayload` の代表 5 ケースを確認 |
| `AssertT05CellFromItem[]` | `iCellFromSlideItem` の Cell 展開と空 Association fallback を確認 |
| `AssertT05CommitHintStructure[]` | `iBuildCommitterHint` の必須マーカーを確認 |
| `AssertT05FallbackGuards[]` | `iDeterministicSlideCommit` の Status ガードを確認 |
| `AssertT06ContentBasedDetection[]` | SlideOutline/SlideDraft キーの拾い上げを確認 |
| `AssertT06SlideDraftExpansion[]` | SlideDraft 形式 item の Cell 展開を確認 |
| `AssertT06SlideOutlineExpansion[]` | SlideOutline 形式 item の Cell 展開を確認 |
| `AssertT07StyleSanitization[]` | `iSanitizeCellStyle` の正規化を確認 |
| `AssertT07SlideIntentDetection[]` | `iDetectSlideIntent` の page count 抽出を確認 |
| `AssertT07InnerCellFromSpecSanitize[]` | 冗長 Style が有効値に落ちることを確認 |
| `AssertT07DefaultPlannerSlideAware[]` | スライド要求で Explore+Draft 分解になることを確認 |
| `AssertT07WorkerPromptSlideHint[]` | T07 SLIDE-MODE prompt 拡張を確認 |
| `AssertT07bResolveTargetNotebookLogic[]` | スライド意図検出ロジックを確認 |

### Phase 32 用

| 関数 | 目的 |
|------|------|
| `AssertPhase32Task31RejectsNaturalLangInput[]` | Commit role での自然言語 Input セルを Deny することを確認 |
| `AssertPhase32Task31AcceptsMathExprInInputCell[]` | 妥当な式は Input セルとして Permit されることを確認 |
| `AssertPhase32Task31AcceptsNaturalLangInTextCell[]` | "Text" セルは検査対象外で Permit されることを確認 |
| `AssertPhase32Task32SkipsShortFactualJa[]` | 短い日本語 factual query が Single 経路に倒れることを確認 |
| `AssertPhase32Task32SkipsShortFactualEn[]` | 短い英語 factual query が Single 経路に倒れることを確認 |
| `AssertPhase32Task32KeepsLongComplex[]` | 長いマルチステップタスクが Orchestrator 経路を保つことを確認 |
| `AssertPhase32Task32KeepsSlideRequest[]` | スライド要求が複雑タスクとして扱われることを確認 |
| `AssertPhase32Task32KeepsSequentialTask[]` | 順序接続語が多い指示が複雑タスクとして扱われることを確認 |
